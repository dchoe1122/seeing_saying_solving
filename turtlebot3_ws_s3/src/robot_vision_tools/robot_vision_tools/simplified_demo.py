...
class Nav2StateMonitor(Node):
    def __init__(self):
        ...
        self.state = 'conflict'  # Start in conflict state for demo purposes
        ...
        #     custom_qos_profile
        # )
        self.issue_description_publisher = self.create_publisher(
            String,
            '/tb1/issue_description',  # resolved under namespace
            custom_qos_profile
        )
    ...
    def image_callback(self, msg):
        if self.state == 'conflict':  # Save image only if the state is 'conflict'
            ...
            self.issue_description_publisher.publish(String(data='Conflict detected, saving image. My current path is completely blocked by the obstacle in the image, and I am a mobile robot without maniupulation capabilities.'))
            # Save issue description as a text file
            issue_text = 'Conflict detected, saving image. My current path is completely blocked by the obstacle in the image, and I am a mobile robot without maniupulation capabilities.'
            with open('latest_issue.txt', 'w') as f:
                f.write(issue_text)
