...
def generate_launch_description():
    ...
    #TO DO: Implement way to define number of robots and create them dynamically
    # Define the robots to be spawned in the simulation
    robots = [

        {'name': 'tb1', 'x_pose': '3.2', 'y_pose': '1.7', 'z_pose': 1.1},
        {'name': 'tb2', 'x_pose': '0.5', 'y_pose': '0.0', 'z_pose': 1.1},
        # ...
        # ...
        ]
    ...
    original_turtlebot_pkg = get_package_share_directory('turtlebot3_gazebo')
    ...
    urdf = os.path.join(
        original_turtlebot_pkg, 'urdf', 'turtlebot3_' + TURTLEBOT3_MODEL + '.urdf'
    )
    # urdf = os.path.join(
    #     turtlebot3_multi_robot, 'urdf','forklift.urdf'
    # )
    #First change the world file to use warehouse world
    # This is the world file that contains multiple robots
    # You can change this to any other world file you want to use
    world = os.path.join(
    ...
    mask_yaml_file = os.path.join(get_package_share_directory('turtlebot3_navigation2'), 'map', 'keepout_mask.yaml')
    param_substitutions = {
    ...
    remappings = [('/tf', 'tf'),
                  ('/tf_static', 'tf_static')]
    filter_mask_server=Node(package='nav2_map_server',
        executable='map_server',
        name='filter_mask_server',
        output='screen',
        # This is the map server node that will load the map - change the map.yaml file to use your own map
    ...
    costmap_filter_info_server=Node(package='nav2_map_server',
    ...
    map_server=Node(package='nav2_map_server',
        ...
        remappings=remappings)
    lifecycle_nodes = ['filter_mask_server', 'costmap_filter_info_server','map_server']
    ...
    for idx, robot in enumerate(robots):
        ...
        # Change arguments for spawn_entity.py between first and second robot
        if idx == 0:
            spawn_args = [
                '-file', os.path.join(original_turtlebot_pkg,'models', 'turtlebot3_' + TURTLEBOT3_MODEL, 'model.sdf'),
                '-entity', robot['name'],
        ...
        # elif idx == 1:
        #     # Example: change z and Y for the second robot
        #     spawn_args = [
        #         '-file', os.path.join(original_turtlebot_pkg,'models', 'turtlebot3_' + TURTLEBOT3_MODEL, 'model.sdf'),
        #         '-entity', robot['name'],
        #         '-robot_namespace', robot['name'],
        ...
        #     ]
            ...
            spawn_args = [
                '-file', os.path.join(turtlebot3_multi_robot,'models', 'turtlebot3_waffle' , 'model.sdf'),
                '-entity', robot['name'],
                '-robot_namespace', robot['name'],
                '-x', robot['x_pose'], '-y', robot['y_pose'],
                '-z', '1.0', '-Y', '0.0',
